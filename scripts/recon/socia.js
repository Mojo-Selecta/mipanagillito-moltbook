...
// ═══════════════════════════════════════════════════════
// Monitors politicians' public activity:
// - Recent tweets from PR politicians (via RSS bridges)
// - Government page change detection
// - Cross-reference: what they say vs what they do
...
const SNAPSHOT_FILE = path.join(__dirname, '..', '.gillito-page-snapshots.json');

// ─── Politician Twitter/X accounts (via RSS bridges) ───

const POLITICIAN_FEEDS = [
  // Using nitter/RSS bridge instances for public tweet feeds
  // These are public RSS feeds of public accounts — no API needed
...
// ─── Government pages to monitor for changes ───
...
async function scanPoliticianFeeds() {
  ...
  for (const pol of POLITICIAN_FEEDS) {
      ...
      try {
        ...
        for (const item of items.slice(0, 10)) {
          ...
          findings.push({
            ...
            subcategory: 'politician_tweet',
            signals: classification.signals,
            headline: `@${pol.handle}: "${item.text.slice(0, 120)}..."`,
            summary: sanitize(item.text),
            source: `Twitter/@${pol.handle}`,
            sourceUrl: item.link || `https://x.com/${pol.handle}`,
            entities: [pol.name],
    ...
    if (!foundFeed) {
      console.log(`      ⚠️ @${pol.handle}: no feed available`);
    }
  }
...
function saveSnapshots(snapshots) {
  try {
    ...
    console.warn(`      ⚠️ Could not save snapshots: ${err.message}`);
  }
}
...
async function scanPageChanges() {
  ...
  for (const page of WATCH_PAGES) {
    try {
      ...
      // Hash the content (strip dynamic elements)
      const cleaned = html
      ...
      if (prevHash && prevHash !== hash) {
        ...
        findings.push({
          category: 'social_listening',
          subcategory: 'page_change',
          signals: [...classification.signals, 'page_changed'],
          headline: `🚨 ${page.name} — Content changed since ${prevChecked || 'last check'}`,
          summary: `Government page "${page.name}" was modified. This could indicate new announcements, policy changes, or removed content. Check: ${page.url}`,
          source: page.name,
          sourceUrl: page.url,
